sap.ui.define(
	...
	function(MessageToast, Controller, ODataModel, JSONModel, List) {
		...
		var PageController = Controller.extend("my.control.library.sample.ListSelection.Page", {
			onInit: function() {
				...
				var oModelProducts = new JSONModel(
					jQuery.sap.getModulePath("my.control.library.sample.ListSelection") +
						"/localService/mockdata/SampleMockdata.json"
				);
				this.getView().setModel(oModelProducts, "sample");

				this.getView().setModel(new JSONModel({ includeItemInSelection: true }), "local");
			},
			onAfterRendering: function() {
				...
				var head = $("head");
				var linkToStyling = jQuery.sap.getModulePath("my.control.library.sample.ListSelection") + "/styling.css";
